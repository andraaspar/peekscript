import { Rational } from './RationalBigInt'
...
test(`[rgplny]`, () => {
	expect(new Rational(714n, 85n).toString()).toBe('(8+2/5)')
})
...
test(`[rgpytk]`, () => {
	expect(new Rational(5n).toThePowerOf(new Rational(3n)).toString()).toBe(
		'(125)',
	)
})
...
test(`[rjlc08]`, () => {
	expect(new Rational(1n, 2n, 1n).toNumber(3)).toBe(0.5)
})

test(`[rjlc3p]`, () => {
	expect(() => new Rational(1n, 3n, 1n).toNumber(3)).toThrow(/convert/)
})
